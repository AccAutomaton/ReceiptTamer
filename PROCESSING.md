...
| 订单管理      | ✅  | “订单列表”使用透明顶栏，筛选条紧接标题区、左缘与标题及账页的 16dp 边线对齐，并以透明背景固定在滚动视口上方；“全部”统一承担筛选重置，不另设“清除筛选”按钮；账页顶部通过内容透明度渐隐延续整页背景，按月连续滚动并保留组内粘性月份栏、日期侧栏与 hairline 行；展示、排序、筛选、搜索及统计统一使用业务日期，缺失时回退收录时间；保留详情编辑和发票关联流程 |
| 发票管理      | ✅  | “发票列表”使用透明顶栏，筛选条紧接标题区、左缘与标题及账页的 16dp 边线对齐，并以透明背景固定在滚动视口上方；“全部”统一承担筛选重置，不另设“清除筛选”按钮；账页顶部通过内容透明度渐隐延续整页背景，按月连续滚动并保留组内粘性月份栏与批量关联订单统计；展示、排序、筛选、搜索及统计统一使用业务日期，缺失时回退收录时间；保留详情编辑和订单关联流程 |
