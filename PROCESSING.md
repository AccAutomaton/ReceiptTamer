...
2. **AI 识别**：用户可选择纯手工、本地 MNN 模型或 OpenAI-compatible 云端模型；本地/云端识别可提取订单截图中的店铺名称、实付款、下单时间、订单号
...
| 报销装订轨视觉系统 | ✅ | 全局 Tinos + Noto Serif SC 衬线体系、扁平控件、连续四栏导航、独立新增侧键、连续月账页、浅暗色与减少动态效果支持；Android 系统栏使用 edge-to-edge 并适配手势导航安全区 |
| 订单管理      | ✅  | “订单列表”使用透明顶栏，筛选条紧接标题区、左缘与标题及账页的 16dp 边线对齐，并以透明背景固定在滚动视口上方；搜索、今日和月份条件持续显示为可清除标签；展示、排序、筛选、搜索及统计统一使用业务日期；保存前按附件哈希、订单号及商家+日期+金额分级提示重复记录 |
| 发票管理      | ✅  | “发票列表”沿用连续月份账页与固定筛选条，搜索、今日和月份条件持续显示为可清除标签；展示、排序、筛选、搜索及统计统一使用业务日期；保存前按附件哈希、发票号及销售方+日期+金额分级提示重复记录 |
| 发票-订单关联   | ✅  | 一对多关系（一张发票关联多个订单）；发票新增、更新、删除与显式关联变更在同一 SQLite 事务提交，任一写入失败整体回滚；已归属其他发票的订单禁止普通勾选，仅能通过“转移关联”查看原发票→目标发票、受影响订单与金额并二次确认 |
...
| 用餐明细导出    | ✅  | Excel格式、按日期/餐时分类、金额分摊、汇总行、自动保存到Download/ReceiptTamer；完整报销生成后与两类 PDF 一并进入逐项结果页 |
| 发票金额分摊    | ✅  | 按订单金额比例分摊、精确无舍入误差                                       |
| 首页工作台    | ✅  | 首次空数据状态显示“导入订单—选择识别方式—关联并导出”的轻量任务清单；“待关联订单”以印章图标、数字摘要和行动箭头组成主入口；最近订单按 `createdAt` 展示最新 10 条 |
| 完整报销流程  | ✅  | 按订单/按发票双依据选择；日期筛选、清除日期和手动刷新保留完整选择，并持续披露筛选范围外已选数量；全选/反选仅作用当前可见项；三类材料生成期间冻结参数并拦截返回，完成页逐项显示结果、打开/预览/分享/重试，支持分享全部与按需 ZIP |
| 待关联发票订单 | ✅  | 原“开票助手”的用户可见名称；默认全部日期，按店铺聚合尚未关联发票的订单，并可批量预选订单新建发票及写入关联 |
| 分享功能      | ✅  | 图片/PDF 分享导入支持“稍后处理”保留当前待办，另设需确认的“放弃全部”；导出结果支持单项分享、分享全部和可选 ZIP |
| PDF预览     | ✅  | syncfusion_flutter_pdfviewer                            |
| 设置        | ✅  | 从主导航移出，由首页齿轮进入；整合模型、存储、清理、备份、隐私、开源、版本、更新与应用信息 |
| 数据清理      | ✅  | 订单/发票清理只允许直选当前筛选可见项；级联带入范围外记录时持续披露隐藏数量并计入完整金额；实际删除期间明确拦截返回，完成结果确认后再离开 |
| OCR引擎     | ✅  | RapidOcrAndroidOnnx；取消识别会使当前请求失效，迟到结果不得回填表单 |
| LLM推理     | ✅  | 纯手工、本地 MNN 或 OpenAI-compatible 云端模型；选择时就地说明速度、隐私和成本，首次向每个云端域名上传前明确显示目标域名并确认 |
...
│   │   ├── app_system_ui.dart     # edge-to-edge 系统栏与浅暗图标策略
...
- 首页空数据状态先显示“导入一笔订单—选择识别方式—关联发票并导出”的三步轻量清单，说明识别功能可选且手工录入可完成全流程。顶部装订目录入口简称“待关联订单”，继续使用整行主入口、数字摘要和显式行动箭头；对应功能页仍使用“待关联发票订单”。用餐证明导出与发票导出并列为两张小卡。下方“最近订单”按 `createdAt` 展示最新 10 条订单。
- 完整报销进入页面即默认“按订单导出”，也可切换“按发票导出”。日期筛选、清除日期和用户手动刷新只改变当前可见清单，不清除已选；底栏持续显示“已选 N 笔，其中 M 笔不在当前筛选范围内”，筛选外记录继续参与数量、金额、关系和最终导出。全选与反选只作用当前可见项。订单、发票、关系、清理或还原发生实际写入后的 revision 重载仍会保留日期条件并清除需要重新确认的选择，且给加载期操作设置串行边界；最终范围始终由本次勾选确定，不创建持久化报销批次。
- 材料生成页接收显式的发票与订单 ID，默认开启用餐证明 PDF、发票 PDF 和用餐明细 XLSX；生成期间冻结本次参数快照并用 `PopScope` 明确拦截返回。完成后不再直接跳普通目录，而是按三类材料分别展示成功、失败或未选择：成功项可打开、调用系统预览和单项分享，失败项可按原快照单独重试；全部成功项可一次分享，也可按需生成 ZIP 后分享。PDF 生成前继续验证附件存在且非空，保存引用同时保留可打开的本地路径与可分享的 Content URI。
- 订单和发票保存前按需计算附件 SHA-256，不新增持久化字段：内容哈希一致是强提示；订单号/发票号一致展示本次与已有记录对比；商家（销售方）+日期+金额一致仅标记“疑似重复”。选择器中的详情统一使用显式箭头，不再要求用户长按或双击。分享导入的当前文件仅在保存成功后出队，直接返回或“稍后处理”都会保留；后续新分享与旧待办合并去重，“放弃全部”必须二次确认。
- 未配置识别方式时，录入页就地比较纯手工、本地模型和云端模型的速度、隐私与成本；从设置返回后若配置完成，可直接继续本次识别。每个云端目标域名首次收到上传前，界面明确显示域名及本次发送的是图片还是提取文字，确认结果作为设备本地偏好保存。OCR 底层任务不支持物理中止时使用请求代次使取消后的结果失效，旧请求不得更新进度、结果或表单；取消后窗口持续显示“正在安全结束”，直到底层任务返回，并明确提示已发出的云端请求仍可能产生费用。
...
- `AppTypography` 的所有 UI 角色统一以 Tinos 处理拉丁字符和数字，以 Noto Serif SC 回退中文；金额保留等宽数字特性。应用在系统无障碍缩放之前为全部界面文字统一增加 1 logical pixel，显式字号与 Material 默认角色一并生效；首页、订单、发票、报销四个主导航页的左上角标题局部保留原字号，图标尺寸不随之改变。MiSans、Noto Sans SC 与 Courier Prime 资产仍为 PDF 或兼容路径保留，但不再作为 UI 主字体。
...
- Android 窗口从原生启动阶段到 Flutter 页面统一使用 edge-to-edge：Android 10+ 走 Flutter/Android 标准路径，Android 8/9 为早期厂商手势导航提供 best-effort 兼容，Android 7.x 保留系统导航栏的安全背景；页面背景延伸到状态栏和受支持的手势导航小白条后方，底部业务控件仍按 `viewPadding`/`SafeArea` 避让。系统栏颜色与分隔线保持透明并关闭 Android 10+ 对比度遮罩，浅色页面使用深色系统图标、暗色及黑底图片预览使用浅色系统图标。小米 HyperOS/MIUI、荣耀 MagicOS、OPPO ColorOS 与 vivo OriginOS/FuntouchOS 共用标准 Android WindowInsets 路径，不依赖不稳定的厂商私有 flag；手势条的最终颜色、形状、是否显示及系统设置仍由 ROM 决定，应用只提供匹配当前页面明暗的外观请求。
- `AppBar` 透明显示连续的页面纸色，标题栏下方不再出现异色横带。`ScrollEdgeFog` 的普通顶/底雾边使用 `Stack`、`IgnorePointer`、实体保护区和线性渐变；订单与发票列表的顶部则使用单个 `dstIn` alpha mask 让账页内容透明淡入，从而保留连续的液态背景且不叠加实色带，刷新指示器位于遮罩外。所有雾边都不使用 `BackdropFilter`；滚动内容在固定顶控件下方淡入，在固定导航、操作栏或批选栏上方完成渐隐，控件占位内不再透出列表。主导航按实际高度、安全区和 8dp 间隔确定淡出落点，`FloatingOverlayLayout` 则按实测控件高度自动放置雾边与保护区。
...
本轮信息架构与视觉迁移调整了主导航、首页内容、设置入口及完整报销步骤。随后的一对多约束修复将数据库从版本 1 升至版本 2：启动时清理同一订单的重复发票关联，保留最近更新/收录的发票，并为 `order_id` 建立唯一索引。本轮可靠性、金额展示、导出性能与 Android edge-to-edge 系统栏适配未再次变更备份包格式、数据库版本或应用版本号；版本 1 备份会先在隔离副本中清理孤立/重复关系并升级，验证通过后才进入还原。报销双依据清单、筛选后保留选择与分享导入待办都只使用 Riverpod/页面进程内状态，不新增持久化字段；重复附件哈希按需计算且不写入数据库，云端域名确认只保存为设备本地偏好，因此现有备份与还原逻辑无需扩展。金额分摊公式与导出归档目录不变，一票多单的展示语义与快捷导出的分摊基数得到统一。备份还原的校验、提交及冲突处理已经加强，不应再视为“逻辑未改变”。
...
- 文件可在系统文件管理器中直接查看
- 完整报销生成后进入逐项结果页；成功材料可打开下载成果、预览会话副本或分享，失败材料可重试，用户也可分享全部或按需打包 ZIP
...
AI 识别按用户设置路由：
1. **未设置**: OCR 入口就地比较纯手工、本地模型和云端模型的速度、隐私与成本；纯手工可直接返回填写，选择模型则进入设置并在配置完成后继续本次识别。
