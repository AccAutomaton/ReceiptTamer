...
if (!executablePath) {
  throw new Error(
    '未找到 Chrome/Edge。请设置 CHROME_PATH 后重新运行设计画板渲染脚本。',
  );
}
...
if (boardFiles.length !== 10) {
  throw new Error(`应找到 10 张画板，当前找到 ${boardFiles.length} 张。`);
}

const renderFiles = [...boardFiles];
if (fs.existsSync(path.join(sourceDir, 'compact-check.html'))) {
  renderFiles.push('compact-check.html');
}
