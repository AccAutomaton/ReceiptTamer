...
const sourceDir = path.join(root, 'docs', 'design', 'hybrid-redesign');
...
if (!executablePath) {
  throw new Error(
    '未找到 Chrome/Edge。请设置 CHROME_PATH 后重新运行晨雾账簿渲染脚本。',
  );
}
...
if (boardFiles.length !== 10) {
  throw new Error(`晨雾账簿应有 10 张画板，当前找到 ${boardFiles.length} 张。`);
}

const renderFiles = [...boardFiles, 'compact-check.html', 'index.html'];
